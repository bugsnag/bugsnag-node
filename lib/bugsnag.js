...
module.exports = Bugsnag = (function() {
  ...
  Bugsnag.configure = function(options) {
    ...
    if (Configuration.autoNotifyUncaught && !unCaughtErrorHandlerAdded) {
      ...
      return process.on("uncaughtException", function(err) {
        return Bugsnag.notify(err, autoNotifyCallback);
      });
    }
  };
  ...
  Bugsnag.requestHandler = function(req, res, next) {
    ...
    dom.on('error', function(err) {
      return this.notify(err, autoNotifyCallback);
    });
    return dom.run(next);
  };
  ...
  Bugsnag.autoNotify = function(options, cb) {
    ...
    dom.on('error', function(err) {
      return Bugsnag.notify(err, options, autoNotifyCallback);
    });
  ...
  shouldNotify = function() {
    return Configuration.notifyReleaseStages && Configuration.notifyReleaseStages.indexOf(Configuration.releaseStage) !== -1 && Configuration.apiKey;
  };

  autoNotifyCallback = function(error, response) {
    if (error) {
      Configuration.logger.error("Bugsnag: error notifying bugsnag.com - " + error);
    }
    if (Configuration.onUncaughtError) {
      return Configuration.onUncaughtError(err);
    }
  };
