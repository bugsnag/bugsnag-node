...
module.exports = Notification = (function() {
  ...
  function Notification(bugsnagError, options) {
    var event, metaData, nsOptions, _ref, _ref1;
    if (options == null) {
      options = {};
    }
    event = {
    ...
    nsOptions = continuationLocalStorage != null ? (_ref = continuationLocalStorage.getNamespace("bugsnag")) != null ? _ref.get("options") : void 0 : void 0;
    if (nsOptions) {
      options = Utils.mergeObjects(Utils.cloneObject(nsOptions), options);
    }
    if (options.userId) {
    ...
    if (options.groupingHash) {
      event.groupingHash = options.groupingHash;
    }
    ...
    if ((options.severity != null) && (_ref1 = options.severity, __indexOf.call(SUPPORTED_SEVERITIES, _ref1) >= 0)) {
    ...
    metaData = Utils.cloneObject(options, {
      except: ["req", "context", "userId", "groupingHash"]
    });
    if (Object.keys(metaData).length > 0) {
      Utils.mergeObjects(event.metaData || (event.metaData = {}), metaData);
    }
