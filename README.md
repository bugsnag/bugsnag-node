...
In order to get round this we recommend you compile the coffeescript file into a javascript file by running `coffee -c filename.coffee` and then running `node filename.js` to execute your app. This is automatable if you use a [Cakefile](http://coffeescript.org/documentation/docs/cake.html), or you can use [Grunt](http://gruntjs.com/).
...
```javascript
bugsnag.notify(new Error("Non-fatal"), {tabName:{username:"bob-hoskins"}});
```
...
By default the notifier will notify Bugsnag of production and development exceptions. If you wish to be notified about production exceptions only, then you can set the notifyReleaseStages option.

```javascript
bugsnag.register("your-api-key-goes-here",{notifyReleaseStages:["production"]});
```
